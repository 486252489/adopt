...
<div id="wrapper">
    ...
    <div id="page-wrapper">
        <div class="row">
            <div class="col-lg-12">
                <h1 class="page-header">流浪宠物团队活动管理</h1>
            </div>
            <!-- /.col-lg-12 -->
        </div>
        <!-- /.row -->
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="myModalLabel">新建流浪宠物团队活动信息</h4>
            </div>
            <div class="modal-body">
                <form class="form-horizontal" id="new_blog_form" enctype="multipart/form-data">
                    <div class="form-group">
                        <label for="new_title" class="col-sm-2 control-label">
                            活动名称
                        </label>
                        <div class="col-sm-10">
                            <input type="text" class="form-control" id="new_title" placeholder="活动名称" name="title">
                        </div>
                    </div>
                    <div class="form-group">
                        <label for="new_cdate" class="col-sm-2 control-label">
                            活动时间
                        </label>
                        <div class="col-sm-10">
                            <input type="date" class="form-control" id="new_cdate" placeholder="活动时间" name="cdate">
                        </div>
                    </div>
                    <div class="form-group">
                        ...
                        <div class="col-sm-10">
                            <input type="text" class="form-control" id="new_remark" placeholder="活动动态" name="remark">
                        </div>
                    </div>
                    <div class="form-group">
                        ...
                        <div class="col-sm-10">
                            <input type="text" class="form-control" id="new_address" placeholder="发起人" name="author">
                        </div>
                    </div>
                    <div class="form-group">
                        ...
                        <div class="col-sm-1">
                         <input type="file" value="上传照片" id="pic" name="file">
                        </div>
                    </div>
<%--                    <div class="form-group">--%>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="myModalLabe">修改流浪宠物团队活动信息</h4>
            </div>
            <div class="modal-body">
                <form class="form-horizontal" id="edit_blog_form" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-10">
                            <input type="text" class="form-control" id="edit_title" placeholder="标题" value="${news.title}" name="title">
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-10">
                            <input type="date" class="form-control" id="edit_actionTime" placeholder="活动时间" value="${news.cdate}" name="cdate">
                        </div>
                    </div>
                    <div class="form-group">
                        ...
                        <div class="col-sm-10">
                            <input type="text" class="form-control" id="edit_address" placeholder="活动动态" value="${news.remark}" name="remark">
                        </div>
                    </div>
                    <div class="form-group">
                        ...
                        <div class="col-sm-10">
                            <input type="text" class="form-control" id="edit_peoples" placeholder="活动人物" value="${news.author}" name="author">
                        </div>
                    </div>
                    <div class="form-group">
                        ...
                        <div class="col-sm-10">
                            <input type="file" id="edit_pic" value="${news.pic}" name="file">
                        </div>
                    </div>
...
<script type="text/javascript">
    ...
    function build_blogs_table(result){
        ...
        $.each(news,function(index,news){
            ...
            var blogIdTd = $("<td></td>").append(news.id);
            var titleTd=$("<td></td>").append(news.title);
            var actionTimed = $("<td></td>").append(news.cdate);
            var addressTd = $("<td></td>").append(news.remark);

            var picTd = $("<td></td>").append($("<img/>").addClass("mySize").attr("src", "/static/images/animal/" + spilt(news.pic)));
            var eventTd=$("<td></td>").append(news.author);
            var editBtn = $("<button></button>").addClass("btn btn-primary btn-sm edit_btn")
                .append($("<span></span>").addClass("glyphicon glyphicon-pencil")).append("修改");
            //为编辑按钮添加一个自定义的属性，来表示当前员工id
            editBtn.attr("edit-id",news.id);
            var delBtn =  $("<button></button>").addClass("btn btn-danger btn-sm delete_btn")
                .append($("<span></span>").addClass("glyphicon glyphicon-trash")).append("删除");
            //为删除按钮添加一个自定义的属性来表示当前删除的员工id
            delBtn.attr("del-id",news.id);
    ...
    $("#blog_save_btn").click(function(){
        ...
        if (title == ""){
            ...
            var blog = document.getElementById("new_blog_form");
            var blogTd = new FormData(blog);
            //2、发送ajax请求保存
            $.ajax({
    ...
    $(document).on("click",".edit_btn",function(){
        ...
        if (currentAdminId == 0){
            ...
            //1、发送ajax,根据id获取用户信息
            $.ajax({
                url:"${path}/news/findById?id="+id,
                type:"GET",
                success:function(result){
                    //填充用户信息
                    //console.log(result);
                    $("#edit_id").val(result.extend.news.id);
                    $("#edit_title").val(result.extend.news.title);
                    $("#edit_actionTime").val(result.extend.news.cdate);
                    $("#edit_address").val(result.extend.news.remark);
                    $("#edit_peoples").val(result.extend.news.author)
                    $("#edit_event").val(result.extend.news.remark);
                    //$("#edit_pic").val(result.extend.news.pic);
                }});
            //2、弹出模态框
            $("#editBlog").modal({
                backdrop:"static"
    ...
    $("#blog_edit_btn").click(function(){
        ...
        if (title == ""){
            ...
            var blog = document.getElementById("edit_blog_form");
            var blogTd = new FormData(blog);

            $.ajax({
                url:"${path}/news/update",
                type:"POST",
                processData: false,  // 告诉jQuery不要去处理发送的数据
                contentType: false, // 告诉jQuery不要去设置Content-Type请求头
                data:blogTd,
    ...
    function to_findByTime(pn,cdate) {
        $.ajax({
            url:"${path}/news/news",
            type:"POST",
            dataType:"json",
            data:{"pn":pn,"cdate":cdate},
            async:"true",
            success:function (result) {
                build_blogs_table(result);
                build_page_info(result);
                build_page_findByTime(result,cdate);
            },
            error:function (result) {
                alert("查询错误")
            }
