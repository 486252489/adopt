...
public class PetController {
    ...
    @RequestMapping("/pets")
    @ResponseBody
    public Message pet(@RequestParam(required = false) String petType,@RequestParam(required = false) Integer state, @RequestParam("pn")Integer pageNum){
        Integer pageSize = 3;
        if(pageNum == null){
            pageNum = 1;
        }
        PageInfo<Pet> pets = petService.allPet(petType, state,pageNum, pageSize);
